...
DissonanceCurve {
	...
	*spectrum { arg spectrum, highInterval = 1902;
		/*@
		desc: Create a new curve based on a computed Spectrum.
		sprectrum: A spectrum
		highInterval: The largest interval to compute in cents. For octave-based systems, you would usually use 1200.
		@*/

		^super.new.init(spectrum.freqs, spectrum.amps, highInterval);
	}
	...
	* fft { arg buffer, size, cutoff = -60, highInterval = 1902, action;
	...
	initFromFFT{ | buffer, size, cutoff = -60, highInterval = 1902, action|
		...
		act = {|spect|
			this.init(spect.freqs, spect.amps.normalizeSum, highInterval);
			action.notNil.if({
				action.value(this)
			});
		};
	...
	+ {|other|
		...
		other.isKindOf(DissonanceCurve).if({
			other = other.spectrum;
		});

		other.isKindOf(Spectrum).if ({
