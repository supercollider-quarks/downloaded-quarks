...
TestCompositeView : UnitTest {

	test_absoluteBounds  { // just put a comp inside a relative comp and check absoluteBounds

		var w,c,d,s,t,bounds,absoluteBounds;
		w = Window.new("TestCompositeView:test_absoluteBounds white comp should be in the middle of blue comp");
	...
	test_allViewsAbsoluteBounds  {
		...
		var w,c,d,s,t,bounds,absoluteBounds;
		w = Window.new("TestCompositeView:test_allViewsAbsoluteBounds");
	...
	test_nestingRelVsAbs {
		...
		vs = [true,false].collect({ |relativeOrigin|
			...
			title = relativeOrigin.if("Relative","Absolute") + "origin should be identical" + TestCompositeView + "test_nestingRelVsAbs";
			w = Window(title, Rect(10 + relativeOrigin.if(600,0), 10, 600, 600));
	...
	test_disappearingContents {
		...
		"TestCompositeView-test_disappearingContents".gui(f);
		f.resizeToFit;
		f.front;
	}
