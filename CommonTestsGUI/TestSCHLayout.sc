...
TestSCHLayoutView : UnitTest {
	...
	test_elasticBackground {
		var q,w,h;
		q = 10;
		w = Window.new("background should be elastic when you resize the window");
	...
	test_flowInRelativeComposite {
		...
		[CompositeView,HLayoutView].do({ |childClass,i|
	...
	test_inComp {
		...
		[Slider,HLayoutView].do({ |class,i|

			var win, gray, white, blue;

			win = Window.new(class.name.asString + "both blue views should be at the top of the white comp ", Rect(100 + (i*410), 100, 400, 300));
			win.view.relativeOrigin = true;

			gray = CompositeView.new(win, Rect(40, 30, 300, 200));
			gray.relativeOrigin = true;
			gray.background = Color.gray;

			white = CompositeView.new(gray, Rect(20, 20, 200, 100));
			white.relativeOrigin = true;
			white.background = Color.white;

			blue = class.new(white, white.bounds.width @ 20);
