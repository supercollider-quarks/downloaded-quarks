...
NanoKtl : MIDIKtl {
	...
	*initClass {
	...
	init {
		...
		orderedCtlNames = ctlNames[1].keys.reject(_ == 'mode').asArray.sort.clump(9)[[2, 3, 1, 0]].flat;
...
//		mastaFunc = Volume.softMasterVol(0.05, server, \midi.asSpec);
			...
			this.mapCCS(scene, key,
				{ |ccval|
					...
					try {
						...
						pxmixers[scene].arGuis[i + pxOffsets[scene]].proxy
		...
		[\bu1, \bu2, \bu3, \bu4, \bu5, \bu6, \bu7, \bu8].do { |key, i|
			this.mapCCS(scene, key, { |ccval|
				defer { if (ccval > 0) {
						pxmixers[scene].arGuis[i + pxOffsets[scene]].edBut.doAction
				} };
			})
		};
		 ...
		 [\bd1, \bd2, \bd3, \bd4, \bd5, \bd6, \bd7, \bd8].do { |key, i|
			this.mapCCS(scene, key,
				{ |ccval| defer {
					...
					var pxGui = pxmixers[scene].arGuis[i + pxOffsets[scene]];
	...
	//	this.mapCCS(scene, \bd9, { |ccval| if (ccval > 0) { this.paramShift(1, scene) } });
		...
		this.mapToNdefGui(mixer.editGui, scene, true);
...
//		mastaFunc = Volume.softMasterVol(0.05, server, \midi.asSpec);
		...
		[\bu1, \bu2, \bu3, \bu4, \bu5, \bu6, \bu7, \bu8].do { |key, i|
		 ...
		 [\bd1, \bd2, \bd3, \bd4, \bd5, \bd6, \bd7, \bd8].do { |key, i|
			this.mapCCS(scene, key,
				{ |ccval| defer {
									////////////// FIXXXXX
					var px = pxmixers[scene].arGuis[i + pxOffsets[scene]].proxy;
					if ( ctlNames[scene]['mode'] == 'push' ){
						if (ccval == 127) {
	...
	pxShift { |step = 1, scene=1|
		...
		var numActive = pxmixers[scene].arGuis.count { |mon| mon.proxy.notNil };
