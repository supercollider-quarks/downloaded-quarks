...
Experiments with coupling of oscillators. 

CoupledSpring -> sc port from a max/msp written by katja vetter

Coupled gravity oscillators with two walls: 
CoupledSpring2 // see test file
CoupledSpring3
 
NCoupling // experiment in describing an 1-dimensional unequal string. 
Simplyfied model: 

Wall - Spring - Mass - Spring - Mass - etc.. etc.. - Spring - Wall

StiffnessArray should aways be 1 element bigger than the input massArray. 

The sclang array can be accessed by specifying the number of the input we want to acces. IN0(n) or IN(n) 
...
void NCoupling_Ctor(NCoupling *unit) {
	...
	// initialize feedback parameters
	int n = IN0(0); // nMasses fixed is also a fixed parameter of initialization of the ugen instance. 
...
void NCoupling_next(NCoupling *unit, int inNumSamples) {
	...
	for (int i = 0; i < inNumSamples; i++) {
		...
		for (int massIndex = 0; massIndex < unit->n; massIndex++) {
			...
			// check if the input assignment is done correctly. 
			stiffnessL = IN0(i + unit->n); 
			stiffnessR = IN0(i + 1 + unit->n);		   
		...
		out[i] = *unit->disp_Array; // output displace of the first mass. 
	};
}
