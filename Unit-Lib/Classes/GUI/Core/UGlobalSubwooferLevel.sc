...
UGlobalSubwooferGain {
	...
	*kr { |use = 1|
		Udef.addBuildSpec( ArgSpec( \u_globalSubwooferGain, UGlobalSubwooferGain, UGlobalSubwooferGain, true ) );
		^(\u_globalSubwooferGain.kr( gain ) * use).dbamp;
	}
	...
	*sendServer { |server|
		RootNode( server ).set( \u_globalSubwooferGain, this );
	}
